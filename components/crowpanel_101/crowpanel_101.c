...
static esp_err_t bsp_enable_panel_power(void) {
  ...
  ESP_LOGI(TAG, "Panel power rail (%d mV) enabled", BSP_PANEL_PWR_LDO_VOLTAGE_MV);
  return ESP_OK;
}
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config,
  ...
  ESP_GOTO_ON_ERROR(
      esp_lcd_new_panel_ek79007(io, &lcd_dev_config, &disp_panel), err, TAG,
...
static lv_display_t *bsp_display_lcd_init(void) {
  ...
  esp_lv_adapter_display_config_t disp_cfg = {
      ...
      // default tear-avoidance mode for this interface.  Requires
      // CONFIG_BSP_LCD_DPI_BUFFER_NUMS=3 in the DPI hardware config.
