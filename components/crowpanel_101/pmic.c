...
static const char *TAG = "stc8h1kxx";
...
{
    return i2c_master_transmit_receive(pmic_dev, &reg, 1, val, 1,
                                       I2C_TIMEOUT_MS);
}
...
esp_err_t bsp_pmic_get_battery_percent(uint8_t *pct)
{
    ...
    uint8_t val = 0;
    ESP_RETURN_ON_ERROR(pmic_read_u8(REG_BAT_LEVEL, &val), TAG,
                        "read bat_level");
    if (val > 100) {
        val = 100;
    }
...
esp_err_t bsp_pmic_get_charge_status(bsp_pmic_chg_t *status)
{
    ...
    uint8_t state = 0;
    ESP_RETURN_ON_ERROR(pmic_read_u8(REG_BAT_STATE, &state), TAG,
                        "read bat_state");
    switch (state) {
    case BAT_CHARGE_CHARGING:
        *status = BSP_PMIC_CHG_CHARGING;
        break;
    case BAT_CHARGE_FULLY_CHARGED:
        *status = BSP_PMIC_CHG_FULL;
        break;
    case BAT_CHARGE_IDLE:
        *status = BSP_PMIC_CHG_DISCHARGING;
        break;
    case BAT_CHARGE_NO_CHARGE:
    case BAT_CHARGE_ERROR:
    default:
...
bool bsp_pmic_is_vbus_present(void)
{
    ...
    uint8_t state = 0;
    if (pmic_read_u8(REG_BAT_STATE, &state) != ESP_OK) {
        return false;
    }
    return (state == BAT_CHARGE_CHARGING) ||
           (state == BAT_CHARGE_FULLY_CHARGED);
}
